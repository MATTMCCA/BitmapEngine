...
namespace aria {
  namespace csv {
    ...
    struct Field {
      ...
      explicit Field(/*const*/ std::string& str) : type(FieldType::DATA), data(&str) {} //removed const, mjm 9/25/2021

      FieldType type;
      /*const*/ std::string* data; //removed const, mjm 9/25/2021
    };
    ...
    private:
      ...
      char* top_token() {
        ...
        if (m_cursor == m_inputbuf_size) {
          ...
          if (m_input.eof()) {
            m_eof = true;
            m_inputbuf_size = (size_t) m_input.gcount(); //added cast, mjm 9/25/2021
        ...
        void next() {
          ...
          for (;;) {
            ...
            switch (field.type) {
              ...
              case FieldType::DATA:
                if (num_fields < m_row.size()) {
                  m_row[num_fields] = std::move(*field.data); //not sure about this move warning, mjm 9/25/2021
                } else {
                  m_row.push_back(std::move(*field.data)); ////not sure about this move warning, mjm 9/25/2021
                }
                num_fields++;
            }
          }
